...
export async function requestAlphaVantageQuoteSnapshot({
  ...
  if (ticker !== ALLOWED_TICKER) {
    return {
      ...
      payload: {
        ...
        errorMessage: `This bounded connector is authorized only for ${ALLOWED_TICKER}.`,
  ...
  requestUrl.searchParams.set("symbol", ALLOWED_TICKER);
  ...
  if (!metadata || !series || providerSymbol !== ALLOWED_TICKER || !latestBarKey || price === null || !timezone) {
